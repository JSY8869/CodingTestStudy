...
class Solution:
    def networkDelayTime(self, times, n, k):
        def dijkstra(a):
            ...
            while heap:
                distance, now = heapq.heappop(heap)         #distance -> 시작점부터 현재 노드까지의 거리, now -> 현재 위치
                if d[now] < distance:
                    continue
                for next, weight in connections[now]:       #next -> 다음 노드, weight -> 두 노드 사이 거리
                    nextdistance = d[now] + weight          #nextdistance -> 시작점에서부터, 현재 노드의 다음 노드까지의 거리
        ...
        d = [INF] * (n + 1)     #d -> 시작점에서 현재 위치까지의 최단 경로를 기록해주는 리스트(초기값은 무한대로 설정)
